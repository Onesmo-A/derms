...
function StatCard({
    ...
    // Map the old CSS background color tones to consistent accent colors for the icon container
    let bgTint = 'bg-[#0F4C81]/10';
    let textStyle = 'text-[#0F4C81]';
    if (tone.includes('emerald')) {
        bgTint = 'bg-emerald-500/10';
        textStyle = 'text-emerald-700';
    } else if (tone.includes('rose')) {
        bgTint = 'bg-rose-500/10';
        textStyle = 'text-rose-700';
    } else if (tone.includes('violet') || tone.includes('purple')) {
        bgTint = 'bg-purple-500/10';
        textStyle = 'text-purple-700';
    } else if (tone.includes('sky') || tone.includes('blue')) {
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
            <div className="flex items-center justify-between">
                <div className="min-w-0">
                    ...
                    {hint ? <p className="mt-1 text-xs text-slate-500 truncate">{hint}</p> : null}
                </div>
                <div className={`rounded-xl ${bgTint} p-3 flex-shrink-0 ${textStyle}`}>
                    {icon}
                </div>
            </div>
        </div>
...
export default function SchoolDetailPage() {
    ...
    return (
        <div className="space-y-4">
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white text-slate-800 shadow-sm transition">
                <div className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
                    <div className="space-y-4">
                        <button
                            onClick={() => navigate('/schools')}
                            className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-[11px] font-semibold text-slate-600 transition hover:bg-slate-100"
                        >
                            <ArrowLeft className="h-3.5 w-3.5" />
                            Back to schools list
                        </button>
                        <div className="space-y-1.5">
                            <p className="text-[10px] font-bold uppercase tracking-[0.28em] text-slate-400">School Management Detail</p>
                            <h1 className="text-2xl font-black tracking-tight text-[#0F4C81] sm:text-3xl">{school.name}</h1>
                            <p className="text-xs text-slate-500">
                                {school.registration_number || 'No reg no'} · {(school.type || 'school').toString()} · {getEnrolmentLabel(school)}
                            </p>
                        </div>
                        <div className="flex flex-wrap gap-2 text-[11px] font-semibold">
                            <span className="rounded-full bg-slate-50 border border-slate-200/80 px-2.5 py-1 text-slate-600">{school.district?.name || 'No district'}{school.district?.region ? `, ${school.district.region.name}` : ''}</span>
                            <span className="rounded-full bg-slate-50 border border-slate-200/80 px-2.5 py-1 text-slate-600">{totalStudents} students</span>
                        </div>
                    </div>
                    
                    {/* Class Summary Table */}
                    <div className="rounded-2xl border border-slate-200/60 bg-slate-50 p-4 shadow-inner sm:min-w-[520px]">
                        <div className="mb-2 flex items-center justify-between gap-3">
                            <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-slate-500">Class Summary</p>
                            <span className="text-[11px] font-bold text-slate-600">{classSummaryTotal} total</span>
                        </div>
                        <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-white">
                            <table className="w-full border-collapse text-left text-[11px] text-slate-700">
                                <thead className="bg-slate-50 text-slate-500 border-b border-slate-200">
                                    <tr>
                                        <th className="px-2.5 py-2 font-bold uppercase tracking-[0.12em]">Type</th>
                                        {displayedSummaryColumns.map((col) => (
                                            <th key={col} className="px-2.5 py-2 font-bold uppercase tracking-[0.12em]">{col}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                                    {summaryRows.map((row, rIdx) => (
                                        <tr key={row.label} className={rIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                                            <td className="px-2.5 py-2 text-xs font-bold uppercase tracking-[0.12em] text-slate-500 border-t border-slate-100">
                                                {row.label}
                                            </td>
                                            {displayedSummaryColumns.map((col) => (
                                                <td key={col} className="px-2.5 py-2 text-sm font-black text-slate-900 border-t border-slate-100">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="mt-3 overflow-x-auto scrollbar-hover">
